...
public class Robot extends IterativeRobot {
	...
	// private final SendableChooser<String> positionChooser = new
	// SendableChooser<>();
	...
	public void autonomousInit() {
		...
		// autonomousCommand = (Command) autoChooser.getSelected(); // dklann: added for
		// SmartDashboard functionality
	...
	public void autonomousPeriodic() {
		...
		/*******
		 * Original autonomous mode code if (m_timer.get()<6) {
		 * m_robotDrive.tankDrive(-.6, -.6); } else if (m_timer.get()<9){
		 * m_robotDrive.tankDrive(-.7, .7); } else if (m_timer.get()<15) {
		 * m_robotDrive.tankDrive(-.6, -.6); } else { m_robotDrive.stopMotor(); } if
		 * (m_timer.get()<6) { m_robotArm.set(1.0); } else { m_robotArm.stopMotor(); }
		 *******/
		...
		// System.out.println(area); //prints area of limelight view target occupies to
		// console, also used to tell robot to stop
		double turningValue = (kAngleSetpoint - m_gyro.getAngle()) * kP;

		try {

			if (m_gyro.getAngle() < 80) {
				m_ControlMethods.Right(.5);
				System.out.println("angle is less than 90");
			} else if (m_gyro.getAngle() < 100) {
				m_ControlMethods.Left(.5);
				System.out.print("angle is more than 90");
			} else {
				m_ControlMethods.Stop();
				System.out.print("stopped motors");
			}
			 ...
			 * if (m_timer.get() > 0) {
			 * 
			 * if(m_timer.get() < 3) { m_ControlMethods.Forwards();
			 * m_ControlMethods.Raisearm(); m_ControlMethods.Closeclaw(); } } else if
			 * (m_timer.get() > 3) m_ControlMethods.Stop(); m_ControlMethods.Stoparm();
			 * m_ControlMethods.Stopclaw(); }
			 */
			/*
			 * if (Location == "Left") {//LEFT POSITION CODE if (Pathid == "11") {//checks
			 * for pathid being 11 System.out.println("location 1, pathid 11"); if
			 * (m_timer.get() > 0) { if (m_timer.get() < 4) {//moves robot forwards and
			 ...
			 * robot forwards if (m_timer.get() < 6) { m_ControlMethods.Forwards(); } } else
			 * if (m_timer.get() > 6) {//turns robot right if (m_timer.get() < 6.3) {
			 * m_ControlMethods.Right(); } } else if (m_timer.get() > 6.3) { //moves robot
			 * forwards to switch and raises arm if (m_timer.get() < 7) {
			 ...
			 * m_ControlMethods.Backwards(); } } else if (m_timer.get() > 12) {//stops robot
			 * and arm if (m_timer.get() < 12.2) { m_ControlMethods.Stop();
			 * m_ControlMethods.Stoparm(); } } else if (m_timer.get() > 12.2) {//opens claw
			 * if (m_timer.get() < 12.4) { m_ControlMethods.Openclaw(); } } else if
			 ...
			 * System.out.println("location 1, pathid 14"); if (m_timer.get() > 0) { if
			 * (m_timer.get() < 4) {//moves robot forwards and grips cube
			 ...
			 * (m_timer.get() > 9.3) {// puts robot in reverse and starts raising arm if
			 * (m_timer.get() < 10) { m_ControlMethods.Raisearm();
			 * m_ControlMethods.Backwards(); } } else if (m_timer.get() > 10) {//turns robot
			 * left if (m_timer.get() < 10.5) { m_ControlMethods.Left(); } } else if
			 * (m_timer.get() > 10.5) {//moves robot forwards if (m_timer.get() < 10.7) {
			 ...
			 * m_ControlMethods.Openclaw(); } } else if (m_timer.get() > 13.6) {//stops claw
			 * if (m_timer.get() < 13.8) { m_ControlMethods.Stopclaw(); } } }//END PATHID 14
			 * CODE else if (Pathid == "22") {//checks for pathid being 22
			 * System.out.println("location 1, pathid 22"); if (m_timer.get() < 4) {//moves
			 * robot forwards and grips cube m_ControlMethods.Forwards();
			 ...
			 * (m_timer.get() > 4.3) {//moves robot forwards if (m_timer.get() < 8) {
			 * m_ControlMethods.Forwards(); } } else if (m_timer.get() > 6) {//turns robot
			 * right if (m_timer.get() < 8.3) { m_ControlMethods.Right(); } } else if
			 * (m_timer.get() > 8.3) { //moves robot forwards to switch and raises arm if
			 ...
			 * m_ControlMethods.Backwards(); } } else if (m_timer.get() > 14) {//stops robot
			 * and arm if (m_timer.get() < 14.2) { m_ControlMethods.Stop();
			 * m_ControlMethods.Stoparm(); } } else if (m_timer.get() > 14.2) {//opens claw
			 * if (m_timer.get() < 14.4) { m_ControlMethods.Openclaw(); } } else if
			...
			// }
			/*
			 * } else if (m_timer.get() > 4.3) {//moves robot forwards if (m_timer.get() <
			 * 8) { m_ControlMethods.Forwards(); } } else if (m_timer.get() > 6) {//turns
			 * robot right if (m_timer.get() < 8.3) { m_ControlMethods.Right(); } } else if
			 * (m_timer.get() > 8.3) { //moves robot forwards to switch and raises arm if
			 * (m_timer.get() < 9) { m_ControlMethods.Forwards();
			 * m_ControlMethods.Raisearm(); } } else if (m_timer.get() > 9) {// stops robot
			 * at switch and opens claw if (m_timer.get() < 9.5) {
			 ...
			 * (m_timer.get() > 11.3) {// puts robot in reverse and starts raising arm if
			 * (m_timer.get() < 12) { m_ControlMethods.Raisearm();
			 * m_ControlMethods.Backwards(); } } else if (m_timer.get() > 12) {//turns robot
			 * left if (m_timer.get() < 12.5) { m_ControlMethods.Left(); } } else if
			 ...
			 * m_ControlMethods.Right(); } } else if (m_timer.get() > 16) {//stops robot and
			 * arm if (m_timer.get() < 16.2) { m_ControlMethods.Stop();
			 * m_ControlMethods.Stoparm(); } } else if (m_timer.get() > 16.2) {//opens claw
			 * if (m_timer.get() < 16.4) { m_ControlMethods.Openclaw(); } } else if
			 ...
			 * forwards and gribs cube if (m_timer.get() < 3) { m_ControlMethods.Forwards();
			 * m_ControlMethods.Closeclaw(); } } else if (m_timer.get() > 3) {//turns robot
			 * left if (m_timer.get() < 3.4) { m_ControlMethods.Left(); } } else if
			 * (m_timer.get() > 3.4) {//moves robot forwards if (m_timer.get() < 5.4) {
			 * m_ControlMethods.Forwards(); } } else if (m_timer.get() > 5.4) {//turns robot
			 * right if (m_timer.get() < 5.8) { m_ControlMethods.Right(); } } else if
			 * (m_timer.get() > 5.8) {//moves robot forwards if (m_timer.get() < 8) {
			 * m_ControlMethods.Forwards(); } } else if (m_timer.get() > 8) {//turns robot
			 * right if (m_timer.get() < 8.4) { m_ControlMethods.Right(); } } else if
			 * (m_timer.get() > 8.4) {//moves robot forwards if (m_timer.get() < 10) {
			 * m_ControlMethods.Forwards(); } } else if (m_timer.get() > 10) {//turns robot
			 * right to face switch if (m_timer.get() < 10.4) { m_ControlMethods.Right(); }
			 * } else if (m_timer.get() > 10.4) {//moves robot forwards to switch and raises
			 * arm if (m_timer.get() < 11) { m_ControlMethods.Forwards();
			 * m_ControlMethods.Raisearm(); } } else if (m_timer.get() > 11) {//opens claw
			 * and stops arm if (m_timer.get() < 11.2) { m_ControlMethods.Openclaw();
			 * m_ControlMethods.Stoparm(); } } else if (m_timer.get() > 11.2) {//stops claw
			 * if (m_timer.get() < 11.4) { m_ControlMethods.Stopclaw(); } } }//END MIDDLE
			 * POSITION CODE
			 * 
			 * if (Location == "Right") {//RIGHT POSITION CODE if (Pathid == "11") {//checks
			 * for pathid being 11 System.out.println("location 3, pathid 11"); } else if
			 * (Pathid == "14") {//checks for pathid being 14
			 ...
			 */
			/*
			 * int Target = 0; if (Target == 1) {//Targeting Procedure if(area >= 12.5) {
			 * m_robotDrive.tankDrive(0, 0); STAHP = 1; //checks distance to target, if
			 * within distance, then stops robot } else if (area >= 12.5) { STAHP = 0; }
			 * if(STAHP == 0) { if(x < -10) { m_robotDrive.tankDrive(0.5, 0.6); //checks for
			 * target to the left, if so, turns robot left } else if(x > 10) {
			 * m_robotDrive.tankDrive(0.6, 0.5); //checks for target to the left, if so,
			 * turns robot } else { m_robotDrive.tankDrive(.6, .6);//default target chase
			 * speed } } }
			 */
	...
	public void teleopPeriodic() {
		...
		// Klann moved this from autonomousPeriodic to see if it works, and it does
		// here!
		/*
		 * if (m_stick.getRawButton(4)) { System.out.println("Button Y"); double
		 * heading_error = -x; double distance_error = -y; double steering_adjust =
		 * 0.0f; if (x > 1.0) { steering_adjust = KpAim*heading_error - min_aim_command;
		 * } else if (x < 1.0) { steering_adjust = KpAim*heading_error +
		 * min_aim_command; }
		 * 
		 * double distance_adjust = KpDistance * distance_error;
		 * 
		 * left_command += steering_adjust + distance_adjust; right_command -=
		 * steering_adjust + distance_adjust; m_robotDrive.tankDrive(left_command,
		 * right_command); }
		 */
		...
		// if(x < -10) {
		// m_robotDrive.tankDrive(0.5, 0.6); // checks for Target being to the left, if
		// so, turns robot left
		// else if(x > 10) {
		// m_robotDrive.tankDrive(0.6, 0.5); //checks for Target being to the right, if
		// so, turns robot right
	}
